...
func _run() -> void:
	...
	for preset in presets:
		...
		# First-launch intro: the guides point at each category.
		SaveData.set_intro_seen(false)
		await _go("start")
		start = _sm().get_node("CurrentState").get_child(0)
		await _wait(3.4)
		_check(start.director.running, "%s: first launch runs the guide intro" % tag)
		start.director.bubble.skip()
		await _wait(0.2)
		start.director.bubble.advanced.emit()
		await _wait(0.4)
		start.director.bubble.skip()
		await _wait(0.2)
		start.director.bubble.advanced.emit()
		await _wait(2.2)
		await _shot("%s_02b_intro_mind" % tag)
		start.director.bubble.skip()
		await _wait(0.2)
		start.director.bubble.advanced.emit()
		await _wait(2.0)
		await _shot("%s_02c_intro_skill" % tag)
		start.director.skip_all()
		await _wait(0.3)
		_check(SaveData.intro_seen(), "%s: finishing the intro marks it seen" % tag)

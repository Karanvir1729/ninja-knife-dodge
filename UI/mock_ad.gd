...
const DURATION := 5.0
const INTERSTITIAL_DURATION := 3.0
...
func run(info: Dictionary) -> bool:
	if bool(info.get("interstitial", false)):
		# Between rounds: no reward, a shorter countdown, and like a real
		# interstitial the close control only appears once it has run.
		_duration = INTERSTITIAL_DURATION
		%Tag.text = "SPONSORED  ·  TEST INTERSTITIAL"
		%Reward.visible = false
		%Claim.text = "CONTINUE"
		%Close.visible = false
		%Blurb.text = "A short ad plays here between rounds in the App Store build. Never in your first rounds, never twice in a row."
	else:
...
func _process(delta: float) -> void:
	if _done:
		return
	_elapsed += delta
	var left := maxf(0.0, _duration - _elapsed)
	%Ring.value = (_elapsed / _duration) * 100.0

extends Node2D
## Star Cricket effects layer: spark bursts, expanding rings and floating
## score text. Positions are in this node's local space (use to_local for globals).
...
## Sparks flung outward from a point. `bias` pushes the spray in a direction.
func burst(at: Vector2, color: Color, count: int = 8, reach: float = 96.0, bias: Vector2 = Vector2.ZERO) -> void:
	for i in count:
		var s := Sprite2D.new()
		...
		add_child(s)
		var dir := Vector2.from_angle(randf() * TAU) * randf_range(reach * 0.4, reach) + bias * randf_range(0.5, 1.0)
		var t := create_tween()
		t.set_parallel(true)

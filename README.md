...
Games beyond the four trials live in **the Yard**: an interlude on the chapter path, a leaderboard tab, results and tour coverage, all driven by a `Globals.GAMES` entry flagged `yard: true` and a `Story.CHAPTERS` entry, with no seal to earn. The first is **Star Cricket** (`cricket/`, `States/cricket_*`): the void bowls, three fielders cover three of the five shot zones so you read the gaps, and your tap's timing sets the shot (perfect is a six regardless, good into a gap is a four, a weak shot into a fielder is caught, a miss on the stumps is bowled), three wickets, an extra-life offer. Its story lives in `story/story.gd` under `YARD`: a cricket team from Japan that prays to Kuro before they play India, and the yard is how he trains them.

## Ads

On iOS the game serves real ads through the Poing Studios AdMob plugin (`addons/admob`, Google Mobile Ads SDK via Swift Package Manager). Two kinds, the usual iPhone mix:

- **Interstitials between rounds.** Shown only when the player leaves a results screen, never during play or a tutorial. Paced by `Ads.gd`: the first two rounds of a session are ad-free, then at most one per three finished rounds, never within two minutes of any ad, and never right after a rewarded video (which counts as an ad). The numbers live in Project Settings under `ninja/ads/interstitial_*`.
- **Rewarded video** behind every "Watch ad" offer (hints, moves, shuffle, hammer, level skip, revive, extra life). Always optional; every reward can also be earned without one.

The ad ids live in Project Settings — `admob/general/ios/app_id`, `ninja/ads/rewarded_unit_id`, `ninja/ads/interstitial_unit_id` — and default to Google's official test ids. TestFlight and Xcode installs always keep the test units; only an App Store install serves the live ones. No tracking prompt is shown and the IDFA is never requested, so ads are non-personalised. On macOS, in the editor and in the debug tour a local offline "test ad" overlay stands in (`UI/mock_ad.tscn`), keeping tests deterministic; the tour switches interstitials off except in `tests/smoke_interstitial.gd`. See `docs/RELEASE.md` for the App Privacy answers.
...
- **The Loop Room** — the music game, reached from the hub's bottom row or Settings > Music vibe. Twenty tunes, each a 6.000 s loop written to one grid (80 BPM, A minor pentatonic) by `tools/gen_loops.py`, so any set of them plays in time: `AudioManager` starts them all together and only moves volume, scaling the mix by 1/sqrt(n) so twenty layers do not clip. A kept mix becomes the YOURS music vibe.
- **Music vibes** — Settings offers KOTO, GARDEN, DUSK (Tozan's CC0 Japanese ambient pieces from OpenGameArt), CLASSIC (the game's original pair) and YOURS (a Loop Room mix). Each supplies the menu, knife and match beds; the cinematic story score is shared. `tools/prep_music.py` rebuilds them all from `sounds/raw/` (hidden from the engine by a `.gdignore`): the CC0 tracks are three-minute pieces, so their tail is crossfaded onto their head to make the loop point inaudible, and every track is normalised onto one target. `AudioManager.TRIM` levels the stragglers; measure with `godot --path . -- --audio=<dir>`, which records the master bus for a full loop of every vibe and slot
...
godot --path . -- --audio=/tmp/ninja-audio      # record the master bus for every music vibe and slot, to measure levels as they actually play
godot --path . -- --room=/tmp/ninja-room        # open The Loop Room, layer tunes on, capture a screenshot and the audio at each step
godot --path . -- --stress=prologue,eye        # play films while hammering them with taps, to catch callbacks that outlive a shot
```
